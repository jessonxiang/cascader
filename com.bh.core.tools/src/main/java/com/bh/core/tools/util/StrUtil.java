...
import com.bh.proprietor.core.tools.lang.StrFormatter;
...
public class StrUtil {
	...
	public String digitToUpperCase(BigDecimal n){
		String b = n.toString();
		String digit = b.split("\\.")[0];
		String decimal = b.split("\\.")[1];

		String nu[]={"零", "壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖"};
		String di[]={"圆", "拾", "佰", "仟","万", "拾", "佰", "仟","亿", "拾", "佰"};
		String de[]={"角", "分"};

		if(digit.length()>11){
			System.out.println("数字太大了，无法输出");
		}
		else{
			String part1 = "";
			for(int i = digit.length(),j = 0;i >0;i--,j++){
				part1 = nu[Integer.parseInt(digit.substring(i-1, i))] + di[j] + part1;
			}
			String part2 = "";
			for(int i = 0;i<decimal.length();i++){
				part2 = part2 + nu[Integer.parseInt(decimal.substring(i,i+1))] + de[i];
			}
