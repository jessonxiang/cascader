...
import com.bh.core.tools.exceptions.UtilException;
import com.bh.proprietor.core.tools.exceptions.UtilException;
...
public class ImageUtil {
	...
	public static String[] imgTypeArr = new String[]{IMAGE_TYPE_GIF,IMAGE_TYPE_JPG,IMAGE_TYPE_JPEG,IMAGE_TYPE_BMP,IMAGE_TYPE_PNG,"svg"};
	/**
	 * 缩放图像（按比例缩放）
	 * 
	 * @param srcImageFile
	 ...
	 *            缩放后的宽度
	 * @param fixedColor
	 *            比例不对时补充的颜色，不补充为<code>null</code>
	 */
			...
			if (null != fixedColor) {// 补白
				...
				if (width == itemp.getWidth(null))
					g.drawImage(itemp, 0, (height - itemp.getHeight(null)) / 2, itemp.getWidth(null),
							itemp.getHeight(null), fixedColor, null);
				else
	...
	public final static void pressImage(File pressImgFile, File srcImageFile, File destImageFile, int x, int y,
		...
		try {
			...
			g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, alpha));
			g.drawImage(pressImg, (wideth - pressImgWidth) / 2, (height - pressImgHeight) / 2, pressImgWidth,
					pressImgHeight, null);
			// 水印文件结束
			g.dispose();
	 ...
	 * @return
	 */
	public static String isImg(String fileName){
		String fileSuffix = fileName.substring(fileName.lastIndexOf(".")+1).toLowerCase();
		boolean contains = Arrays.asList(imgTypeArr).contains(fileSuffix);
		if(contains){
			return "1";
		}else{
			return "0";
